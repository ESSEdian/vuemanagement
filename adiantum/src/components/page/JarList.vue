<template>
  <div class="list">
    <el-table :data="tableData" style="width: 100%">
       <el-table-column label="文件编号" width="300">
      ...
      <el-table-column label="类名" width="300">
        <template slot-scope="scope">
          <!-- <i class="el-icon-time"></i> -->
          <span style=" text-align: center;">{{ scope.row.className }}</span>
        </template>
      </el-table-column>
      <el-table-column label="文件路径" width="300">
        <template slot-scope="scope">
          <!-- <i class="el-icon-time"></i> -->
          <span style=" text-align: center;">{{ scope.row.resourcePath }}</span>
        </template>
      </el-table-column>
      <el-table-column label="文件夹名" width="200">
        <template slot-scope="scope">
          <span style=" text-align: center;">{{scope.row.dir}}</span>
        </template>
      </el-table-column>
      <el-table-column label="createTime" width="300">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" width="400">
        <template slot-scope="scope">
          <!-- <el-button size="mini" @click="opentask">编辑</el-button> -->
          <!-- <el-button size="mini" @click="open4">查看</el-button> -->
          <el-button size="mini" type="danger" @click="stopJar($event,scope.row.id)">禁止</el-button>
          <el-button size="mini" type="danger" @click="deleteJar($event,scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 弹窗 -->
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$http.get(this.$api.fileList).then(res => {
