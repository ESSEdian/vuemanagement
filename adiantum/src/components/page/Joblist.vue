<template>
  <div class="list">
    <el-table :data="tableData" style="width: 100%">
      <el-table-column label="工作编号" width="300">
        <template slot-scope="scope">
          <!-- <i class="el-icon-time"></i> -->
          <span style=" text-align: center;">{{ scope.row.id}}</span>
        </template>
      </el-table-column>
      <el-table-column label="工作名称" width="300">
        <template slot-scope="scope">
          <!-- <i class="el-icon-time"></i> -->
          <span style=" text-align: center;">{{ scope.row.jobName}}</span>
        </template>
      </el-table-column>
      <el-table-column label="工作组" width="300">
        <template slot-scope="scope">
          <!-- <i class="el-icon-time"></i> -->
          <span style=" text-align: center;">{{ scope.row.jobGroup }}</span>
        </template>
      </el-table-column>
      <el-table-column label="工作状态 " width="300">
        <template slot-scope="scope">
          <span style=" text-align: center;">{{scope.row.status==1?'新增':scope.row.status==2?'加入过schedule':scope.row.status==3?'删除':scope.row.status==null?'暂无状态':''}}</span>
        </template>
      </el-table-column>
      <el-table-column label="触发任务名" width="300">
        <template slot-scope="scope">
          <span style=" text-align: center;">{{ scope.row.triggerName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="工作描述" width="300">
        <template slot-scope="scope">
          <span style=" text-align: center;">{{ scope.row.description }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="Action" width="500">
        <template slot-scope="scope">
          <el-button size="mini" type="primary" @click="updateJob($event,scope.row.id)">编辑</el-button> 
          <el-button size="mini" type="primary" @click="findJob($event,scope.row.id)">查看</el-button> 
          <el-button size="mini" type="danger"  @click="stopJob($event,scope.row.id)">禁止</el-button>
          <el-button size="mini" type="danger"  @click="deleteJob($event,scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 弹窗 -->
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$http.get(this.$api.jobList).then(res => {
